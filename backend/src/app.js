...
app.get('/spatium-checkin', (req, res) => {
  res.sendFile(path.join(LANDING_DIR, 'spatium-urgent-care-checkin.html'));
});
...
app.get(/^\/(?!api).*/, (req, res, next) => {
  ...
  if (req.path === '/assessment' || req.path.startsWith('/assessment/')) {
    return next();
  }
