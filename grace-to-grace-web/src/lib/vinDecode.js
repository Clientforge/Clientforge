import { VIN_DECODE_BASE } from '../constants.js';
...
/**
 * @returns {string|null} Decode URL or null if VIN invalid
 */
export function getVinDecodeUrl(vin) {
  const v = normalizeVin(vin);
  if (!isValidVinFormat(v)) return null;
  if (import.meta.env.DEV) {
    return `/vpic-api/vehicles/DecodeVinValues/${encodeURIComponent(v)}?format=json`;
  }
  const base = VIN_DECODE_BASE.replace(/\/$/, '');
  if (base) {
    return `${base}/vin-decode/${encodeURIComponent(v)}`;
  }
  /* Same host as ClientForge API (e.g. app deployed with backend) */
...
/**
 * Map NHTSA DecodeVinValues first row → app shape
 */
